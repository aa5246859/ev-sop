...
(function () {
  ...
  /* ---- API via JSONP ---- */
  var _cbMap = {};
  window._gsCallback = function(id, data) {
    if (_cbMap[id]) { _cbMap[id](data); delete _cbMap[id]; }
  };
  ...
  function apiCall(params) {
    return new Promise(function(resolve) {
      ...
      _cbMap[id] = resolve;
      var url = API_URL + '?callback=_gsCallback&cbid=' + id;
      for (var k in params) {
      ...
      s.src = url;
      s.onerror = function() { resolve({ ok: false, error: 'network error' }); };
      document.head.appendChild(s);
      setTimeout(function() {
        if (_cbMap[id]) { delete _cbMap[id]; resolve({ ok: false, error: 'timeout' }); }
      }, 15000);
    });
  }
  ...
  function apiDel(action, id) {
    return apiCall({ action: action, id: id });
  }
